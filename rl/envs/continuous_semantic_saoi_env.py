...
class ContinuousSemanticSAoIEnv(SingleUAVOFDMAAoIEnv):
    ...
    - a_x, a_y in [-1, 1]: movement direction
    - a_c in [0, 1]: charge bias
    - a_u in [0, 1]: user association
    - a_r in [0, 1]: semantic compression-ratio control
    """
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, Dict]:
        ...
        compression_ratio = self._decode_compression_ratio(action[4])
        ...
        info = {
            ...
            "action_semantic_ratio": float(np.clip(action[4], 0.0, 1.0)),
            **energy_info,
        }
