...
from rl.envs import ContinuousSemanticBaseSAoIEnv, ContinuousSemanticBaseSAoIEnvConfig
...
def heuristic_warmup_action(env, obs, noise_std_xy, noise_std_charge, noise_std_ratio):
    base_action = continuous_rule_semantic_policy(env, obs).astype(np.float32)
    base_action = base_action[[0, 1, 2, 4]]
    noise = np.array(
        [
...
def train(args: argparse.Namespace) -> None:
    ...
    env = ContinuousSemanticBaseSAoIEnv(
        ContinuousSemanticBaseSAoIEnvConfig(max_steps=args.max_steps, num_ues=args.num_ues, seed=args.seed)
    )
    agent = SACAgent(
    ...
    csv_path = ensure_unique_path(args.output_dir, "semantic_base_sac_training_metrics", ".csv")
    best_model_path = ensure_unique_path(args.model_output_dir, "semantic_base_sac_best", ".pth")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
        writer = csv.DictWriter(
            csv_file,
            fieldnames=[
                "episode","reward","reward_ma20","avg_saoi","avg_saoi_ma20","avg_aoi","avg_aoi_ma20","final_aoi",
                "success_updates","service_attempts","success_rate","charge_steps","final_energy","min_energy",
                "queue","final_queue","max_queue","alpha","actor_loss","critic_loss","best_model_path",
            ],
        )
        writer.writeheader()
        ...
        for episode in range(1, args.episodes + 1):
            ...
            episode_reward = 0.0
            mean_saois, mean_aois = [], []
            success_updates = service_attempts = charge_steps = 0
            ...
            while not done:
                ...
                if global_step <= args.start_steps:
                    action = heuristic_warmup_action(env, obs, args.warmup_noise_xy, args.warmup_noise_charge, args.warmup_noise_ratio)
                elif episode <= args.guided_episodes and env.rng.random() < args.guided_exploration_prob:
                    action = heuristic_warmup_action(env, obs, args.guided_noise_xy, args.guided_noise_charge, args.guided_noise_ratio)
                else:
            ...
            if reward_ma20 >= best_reward_ma20:
                ...
                print(f"saved_model(best_semantic_base_sac_checkpoint)={best_model_path}")
            writer.writerow(row)
            csv_file.flush()
            ...
            print(
                ...
                f"reward_ma20={reward_ma20:.3f} "
                f"avg_saoi={avg_mean_saoi:.3f} "
                f"avg_aoi={avg_mean_aoi:.3f} "
    ...
    print(f"saved_csv(semantic_base_sac_training_metrics)={csv_path}")
    print(f"saved_model(semantic_base_sac_checkpoint)={best_model_path}")
    if args.auto_plot:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train semantic SAC baseline without user association action")
