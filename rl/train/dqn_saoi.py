...
from rl.envs import SAoIEnvConfig, SingleUAVSAoIEnv
...
def train(args: argparse.Namespace) -> None:
    ...
    env = SingleUAVSAoIEnv(SAoIEnvConfig(max_steps=args.max_steps, num_ues=args.num_ues, seed=args.seed))
    csv_path = build_unique_csv_path(args.output_dir, args.csv_name)
    best_model_path = ensure_unique_path(args.model_output_dir, "dqn_saoi_best", ".pth")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for episode in range(1, args.episodes + 1):
            ...
            print(
                ...
                f"final_saoi={info['mean_saoi']:.3f} "
                f"avg_aoi={info['mean_aoi']:.3f} "
            ...
            if row["best_model_path"]:
                print(f"saved_model(best_dqn_saoi_checkpoint)={best_model_path}")

    print(f"saved_csv(saoi_training_metrics)={csv_path}")
    print(f"saved_model(dqn_saoi_checkpoint)={best_model_path}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a simple DQN on the SAoI-energy environment")
    ...
    parser.add_argument("--csv-name", type=str, default="saoi_training_metrics.csv")
