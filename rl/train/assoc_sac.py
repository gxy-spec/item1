...
from rl.baselines.continuous_assoc_env import continuous_rule_assoc_continuous_policy
...
def heuristic_warmup_action(
    ...
    base_action = continuous_rule_assoc_continuous_policy(env, obs).astype(np.float32)
    noise = np.array(
        [
...
def train(args: argparse.Namespace) -> None:
    ...
    env = ContinuousAssocSingleUAVAoIEnv(
        ContinuousAssocAoIEnvConfig(max_steps=args.max_steps, num_ues=args.num_ues, seed=args.seed)
    )
    agent = SACAgent(
    ...
    csv_path = ensure_unique_path(args.output_dir, "assoc_sac_training_metrics", ".csv")
    best_model_path = ensure_unique_path(args.model_output_dir, "assoc_sac_best", ".pth")
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as csv_file:
        ...
        for episode in range(1, args.episodes + 1):
            ...
            if saved_best:
                print(f"saved_model(best_assoc_sac_checkpoint)={best_model_path}")

    print(f"saved_csv(assoc_sac_training_metrics)={csv_path}")
    print(f"saved_model(assoc_sac_checkpoint)={best_model_path}")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Train a SAC baseline with user association actions")
